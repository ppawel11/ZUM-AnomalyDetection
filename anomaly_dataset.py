...
class BreastCancerDataset(AnomalyDataset):
    def __init__(self, filename, malignant_percentage_drop=0.0, normalise_data=False):
        super().__init__()
        ...
        drop_index = np.random.choice(
            malignant_entries,
            size=int(len(malignant_entries) * malignant_percentage_drop),
            replace=False,
        )
        self.data = self.data.drop(drop_index).astype(int)

    def normalise_data(self):
        cond = self.data.columns != "class"
        scaler = sklp.StandardScaler().fit(self.data.loc[:, cond])
        self.data.loc[:, cond] = scaler.transform(self.data.loc[:, cond])
    ...
    def split(self, training_percentage):
        ...
        anomalies_df_train_idx = np.random.choice(
            anomalies_df_idx,
            size=int(len(anomalies_df_idx) * training_percentage),
            replace=False,
        )
        anomalies_df_train = anomalies_df[
            anomalies_df.index.isin(anomalies_df_train_idx)
        ]
        anomalies_df_test = anomalies_df.drop(anomalies_df_train_idx)

        normal_df_idx = normal_df.index.tolist()
        normal_df_train_idx = np.random.choice(
            normal_df_idx,
            size=int(len(normal_df_idx) * training_percentage),
            replace=False,
        )
        ...
        x_train = (
            pd.concat(
                [
                    anomalies_df_train.drop(["class"], axis=1),
                    normal_df_train.drop(["class"], axis=1),
                ]
            )
            .sample(frac=1)
        ...
        y_train = (
            pd.concat([anomalies_df_train["class"], normal_df_train["class"]])
            .sample(frac=1)
            .reset_index(drop=True)
            .values.tolist()
        )
        x_test = (
            pd.concat(
                [
                    anomalies_df_test.drop(["class"], axis=1),
                    normal_df_test.drop(["class"], axis=1),
                ]
            )
            .sample(frac=1)
            .reset_index(drop=True)
        ...
        y_test = (
            pd.concat([anomalies_df_test["class"], normal_df_test["class"]])
            .sample(frac=1)
            .reset_index(drop=True)
            .values.tolist()
        )
...
class WineDataset(AnomalyDataset):
    def __init__(self, filename, class_to_drop, drop_percentage, normalise_data=False):
        super().__init__()
        self.data = pd.read_csv(
            filename,
            header=None,
        ...
        drop_index = np.random.choice(
            anomaly_entries,
            size=int(len(anomaly_entries) * drop_percentage),
            replace=False,
        )
    ...
    def split(self, outlier_class=1, training_percentage=0.7):
